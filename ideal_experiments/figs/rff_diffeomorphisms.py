...
splines_all_dims = [
    RBFSampler(gamma="scale", n_components=P_FEATURES) 
...
for i in range(n_plots):
    ...
    model = make_pipeline(
        RBFSampler(n_components=P_FEATURES), 
        StandardScaler(),
        PCA(n_components=P_FEATURES),
        Lasso(alpha=1e-3, max_iter=5000, tol=1e-3))
    model.fit(
...
for a in [0.005, 0.01, 0.1, 0.5, 0.001]:
    estimator = FeaturePermutationEstimator(
        regularizer="group", 
        optim_kwargs={"alpha": a, "scale_reg": None, "l1_reg": 0}, 
...
save_fig(fig, save_dir="figs/features/rff_diffs")