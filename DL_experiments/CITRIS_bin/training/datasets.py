...
class ConceptDataset(data.Dataset):
    def __init__(self, image_dataset, concept_dataset, label_dataset, tabular=False, indexed=True):
    ...
    def __getitem__(self, idx):
        if self.indexed:
            if self.tabular:
                img = self.img_dataset[idx][0].to(torch.float32)
            else:
                img = self.img_dataset[idx].to(torch.float32)
                
            concept = self.concept_dataset[idx][0].to(torch.float32)
            label = self.label_dataset[idx][0].to(torch.float32)
        else:
            img = self.img_dataset[idx]
            concept = self.concept_dataset[idx]
            label = self.label_dataset[idx]
        return img, concept, label
